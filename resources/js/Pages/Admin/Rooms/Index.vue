...
const loadRooms = async () => {
    try {
        ...
        const response = await axios.get('/admin/rooms', { params });
...
const openBuildingDetail = async (building) => {
    selectedBuilding.value = building;
    showDetailModal.value = true;
    await loadRooms(); // Recargar para ver solo las aulas del edificio
};
...
const addRoom = async () => {
    try {
        const response = await axios.post('/admin/rooms', {
            building_id: parseInt(newRoom.value.building_id),
...
const updateRoom = async () => {
    try {
        const response = await axios.put(`/admin/rooms/${editRoom.value.id}`, {
            building_id: parseInt(editRoom.value.building_id),
            nombre: editRoom.value.nombre,
            codigo: editRoom.value.codigo,
            tipo: editRoom.value.tipo,
        });

        const index = rooms.value.findIndex(r => r.id === editRoom.value.id);
        if (index !== -1) {
...
const filteredRooms = () => {
    ...
    if (searchQuery.value) {
        ...
        result = result.filter(room => 
            room.nombre.toLowerCase().includes(query) ||
            room.codigo.toLowerCase().includes(query) ||
            room.building?.nombre.toLowerCase().includes(query)
        );
    }
...
<template>
    <Head title="Gestión de Aulas y Laboratorios - Administrador UTM" />
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p :class="['text-lg', darkMode ? 'text-gray-400' : 'text-gray-600']">
                                Crear y administrar aulas, laboratorios y espacios académicos
                            </p>
                        </div>
                        <button 
                            @click="showAddModal = true"
                            :class="['mt-4 md:mt-0 px-6 py-3 rounded-xl font-semibold transition-all hover:shadow-lg', darkMode ? 'bg-teal-600 hover:bg-teal-700 text-white' : 'bg-teal-500 hover:bg-teal-600 text-white']"
                        >
                ...
                <div :class="['rounded-2xl border shadow-lg overflow-hidden', darkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200']">
                    ...
                    <div v-if="loading" class="p-12 text-center">
                        <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2" :class="darkMode ? 'border-teal-400' : 'border-teal-600'"></div>
                        <p :class="['mt-4', darkMode ? 'text-gray-400' : 'text-gray-600']">Cargando aulas/laboratorios...</p>
                    </div>

                    <!-- Empty state -->
                    <div v-else-if="filteredRooms().length === 0" class="p-12 text-center">
                        ...
                        <p :class="['text-lg font-semibold', darkMode ? 'text-gray-400' : 'text-gray-600']">
                            {{ searchQuery ? 'No se encontraron aulas/laboratorios' : 'No hay aulas/laboratorios registrados' }}
                        </p>
                        <button 
                            v-if="!searchQuery"
                            @click="showAddModal = true"
                            :class="['mt-4 px-6 py-3 rounded-xl font-semibold transition-all', darkMode ? 'bg-teal-600 hover:bg-teal-700 text-white' : 'bg-teal-500 hover:bg-teal-600 text-white']"
                        >
                            Crear Primera Aula/Laboratorio
                        </button>
                    </div>
                    ...
                    <div v-else class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y" :class="darkMode ? 'divide-gray-700' : 'divide-gray-200'">
                                ...
                                >
                                    <td :class="['px-6 py-4 whitespace-nowrap text-sm font-semibold', darkMode ? 'text-teal-400' : 'text-teal-600']">
                                        {{ room.codigo }}
                                    </td>
                                    ...
                                    <td :class="['px-6 py-4 whitespace-nowrap']">
                                        <span :class="['inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold', darkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-800']">
                                            {{ room.tipo }}
                                        </span>
                                    </td>
                                    <td :class="['px-6 py-4 whitespace-nowrap text-sm', darkMode ? 'text-gray-300' : 'text-gray-700']">
                                        {{ room.building?.nombre || 'N/A' }}
                                    </td>
                    ...
                    <div :class="['rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto', darkMode ? 'bg-gray-800' : 'bg-white']">
                        ...
                        <div class="p-6">
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option v-for="building in buildings" :key="building.id" :value="building.id">
                                            {{ building.nombre }} ({{ building.codigo }})
                                        </option>
                                    </select>
                                </div>
                            ...
                            <div class="mt-8 flex justify-end space-x-4">
                                ...
                                <button
                                    @click="addRoom"
                                    :class="['px-6 py-3 rounded-lg font-semibold transition-colors', darkMode ? 'bg-teal-600 hover:bg-teal-700 text-white' : 'bg-teal-500 hover:bg-teal-600 text-white']"
                                >
